...
class EDA_LLM_Client:
    ...
    def generate_spec(self, user_request: str) -> HardwareSpec | str:
        ...
        prompt = (
            ...
            "Return the specification as valid JSON that conforms to the "
            "schema the tool enforces. Use lowercase_with_underscores for "
            "the module_name. Every port must have a name and a numeric width."
        )
        try:
    ...
    def generate_variations(self, spec: HardwareSpec | dict, num_variations: int = 3):
        """Takes a HardwareSpec (or raw dict) and generates N distinct
        SystemVerilog architectural variations.
        """
        ...
        variations = []
        variation_seeds = [
            "Implementation Strategy: Write standard, highly readable behavioral RTL "
            "using always_comb and case statements.",
            "Implementation Strategy: Focus on lowest possible gate count and area "
            "utilization. Share logic where possible.",
            "Implementation Strategy: Focus on maximum performance and minimum "
            "gate-delay latency, even if it costs more area.",
        ]
        for i in range(num_variations):
            seed = (
                variation_seeds[i]
                if i < len(variation_seeds)
                else "Implementation Strategy: Generate a unique structural variation."
            )
